...
class DeterministicGenerator:
    ...
    def __init__(self, seed: int = 42) -> None:
        self.seed = seed
        self._rng = random.Random(seed)  # noqa: S311  # deterministic seeded generator for synthetic data, not crypto
        self._base_time = datetime(2025, 1, 1, tzinfo=UTC)
            ...
            records.append(
                {
                    ...
                    "version": (
                        f"{self._rng.randint(1, 5)}.{self._rng.randint(0, 20)}."
                        f"{self._rng.randint(0, 99)}"
                    ),
                    "config": {
                    ...
                    "findings": [
                        {
                            ...
                            "recommendation": (
                                f"Review and remediate {self._random_resource()} configuration"
                            ),
                        }
            ...
            records.append(
                {
                    ...
                    "compliance_percentage": round(resolved / total_issues * 100, 1)
                    if total_issues > 0
                    else 100.0,
                    "categories": {
