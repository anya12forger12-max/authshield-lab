...
class AuthenticationAttemptRepository(BaseRepository[AuthenticationAttempt]):
    ...
    async def get_by_user(self, user_id: str, page: int = 1, per_page: int = 20) -> dict:
        ...
        count_stmt = select(func.count()).select_from(AuthenticationAttempt).where(base)
    ...
    async def count_failed_attempts(self, user_id: str, since_minutes: int = 30) -> int:
    ...
    async def get_by_correlation_id(self, correlation_id: str) -> list[AuthenticationAttempt]:
