...
class TestClassroomService:
    def test_create_classroom(self):
        ...
        c = service.create_classroom(
            {"name": "CS101", "description": "Intro to CS", "capacity": 50, "instructor_id": "ins1"}
        )
        assert c["name"] == "CS101"
...
class TestEnrollmentService:
    ...
    def test_activate_enrollment(self):
        ...
        repo.get_by_id = MagicMock(
            return_value={"id": "e1", "learner_id": "l1", "status": "pending"}
        )
    ...
    def test_drop_enrollment(self):
        ...
        repo = MagicMock()
        repo.get_by_id = MagicMock(
            return_value={"id": "e1", "learner_id": "l1", "status": "active"}
        )
...
class TestGradebookService:
    ...
    def test_add_grade_item(self):
        ...
        repo.add_grade_item = MagicMock(
            return_value={"id": "gi1", "name": "Midterm", "category": "exam"}
        )
        service = GradebookService(repo)
...
class TestCompetencyService:
    def test_create_competency(self):
        ...
        repo = MagicMock()
        repo.create_competency = MagicMock(
            return_value={"id": "comp1", "name": "Secure Coding", "level": "advanced"}
        )
        service = CompetencyService(repo)
        c = service.create_competency(
            {"name": "Secure Coding", "domain": "Cybersec", "level": "advanced"}
        )
